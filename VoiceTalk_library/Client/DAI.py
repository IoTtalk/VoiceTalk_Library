...
import DAN
# 原本 SA 是直接讀取或是以參數的方式讀取，
# 但現在使用 compile() 跟 exec() 執行 DAI.py，會先在主程式使用 compile() 跟 exec() 執行 SA.py，再把 name_space 放到 DAI 的全域變數中，所以這裡會先檢查全域變數是否有 SA 可用。
if "SA" not in globals():
    SA_module_name = 'SA'
...
#     """處理從標準輸入接收的資料"""
#     while True:
#         line = sys.stdin.readline().strip()  # 從標準輸入讀取一行資料
#         if line:
#             data = json.loads(line)
#             print(f"{device_name} Received data: {line}")  # 這裡可以改成處理資料的邏輯
